...
const UpdateProfile = () => {
  ...
  const profilePicSetter = (e: HTMLFormElement) => {
    if (e.target.files.length === 0 || e.target.files[0] === undefined) {
    ...
    { alert && <MessageBox message={alert}/> }
      ...
      <form className="w-full lg:w-7/12 grid grid-cols-1 md:grid-cols-2 gap-5 mx-auto p-5 rounded-md" onSubmit={(e) => handleUpdateProfile(e)}>
        ...
        <div className="flex flex-col justify-center p-5 bg-white rounded-md shadow-md hover:shadow-xl duration-300 space-y-1">
          ...
          <input hidden className="px-3 py-2 border-2 hover:border-sky-500 text-lg rounded-md font-plex-sans-medium" type="file" accept="image/*" id="photo" onChange={e => profilePicSetter(e)}/>
        </div>
